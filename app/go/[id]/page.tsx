...
export default async function GoPage({{ params }}: Props) {{
  ...
  const params2 = new URLSearchParams({{
    trs: marker, shmarker: marker, locale: 'fr', curr: 'eur', origin: 'BSL',
    one_way: 'false', color_button: 'C9A0DC', color_icons: 'C9A0DC',
    plain: 'true', powered_by: 'true', widget: 'search-form', host: 'www.aviasales.fr',
  }});
  if (iata) params2.set('destination', iata);
  if (deal.departure_date) params2.set('depart_date', deal.departure_date);
  if (deal.return_date) params2.set('return_date', deal.return_date);
  const widgetSrc = 'https://tp.media/content?' + params2.toString() + '&charset=utf-8';
  ...
  return (
    <div style={{{{fontFamily:'sans-serif',background:'linear-gradient(135deg,#f5f0ff 0%,#fce4ec 100%)',minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:'2rem'}}}}>
      <div style={{{{background:'white',borderRadius:'24px',padding:'2.5rem',maxWidth:'560px',width:'100%',boxShadow:'0 20px 60px rgba(0,0,0,0.12)',textAlign:'center'}}}}>
        ...
        <h1 style={{{{color:'#2C3E50',fontSize:'1.4rem',marginBottom:'0.25rem',fontWeight:800}}}}>{deal.destination}</h1>
