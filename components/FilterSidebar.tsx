    ...
    <div className="flex gap-6 w-full">
      <aside className="w-64 shrink-0">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sticky top-20">
          ...
          <div className="mb-5">
            <label className="block text-xs font-bold text-gray-500 mb-2">Budget max — {budgetMax}€</label>
            ...
            <div className="flex justify-between text-xs text-gray-400 mt-1"><span>50€</span><span>3000€</span></div>
          </div>
          <div className="mb-5">
          ...
          <div className="mb-5">
            <label className="block text-xs font-bold text-gray-500 mb-2">Durée</label>
      ...
      <div className="flex-1">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-sm text-gray-500">{filtered.length} deal{filtered.length > 1 ? 's' : ''} trouvé{filtered.length > 1 ? 's' : ''}</span>
        </div>
        {filtered.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
            <p className="text-4xl mb-3">✈️</p>
            <p className="text-lg font-semibold text-gray-600">Aucun deal pour ces critères</p>
            <p className="text-sm text-gray-400 mt-1">Essayez un budget plus élevé ou d&apos;autres filtres</p>
          </div>
        ) : (
